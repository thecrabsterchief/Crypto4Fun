# Reference: https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.197.pdf

Sbox = (
    
)

Inv_Sbox = (
    
)

Rcon = (
    0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36, 0x6C,
    0xD8, 0xAB, 0x4D, 0x9A, 0x2F, 0x5E, 0xBC, 0x63, 0xC6, 0x97, 0x35, 0x6A,
    0xD4, 0xB3, 0x7D, 0xFA, 0xEF, 0xC5, 0x91, 0x39
)

def RotWord(word: bytes) -> bytes:
    """Rotate word by 1 byte to the left."""
    assert len(word) == 4
    return word[1:] + word[:1]

def SubWord(word: bytes) -> bytes:
    """Substitute each byte in word with Sbox."""
    assert len(word) == 4
    return bytes([Sbox[b] for b in word])

def reverse_rounds_key(round_key: bytes, n_rounds: int):
    """Reverse the key schedule to get the original key."""
    assert len(round_key) * 8 == 128

    xor_bytes = lambda x, y: bytes([a ^ b for a, b in zip(x, y)])
    for i in range(n_rounds - 1, -1, -1):
        # current columns
        cur_col1 = round_key[0x00:0x04]
        cur_col2 = round_key[0x04:0x08]
        cur_col3 = round_key[0x08:0x0c]
        cur_col4 = round_key[0x0c:0x10]

        # previous columns
        pre_col4 = xor_bytes(cur_col4, cur_col3)
        pre_col3 = xor_bytes(cur_col3, cur_col2)
        pre_col2 = xor_bytes(cur_col2, cur_col1)
        pre_col1 = xor_bytes(cur_col1, 
                            xor_bytes(RotWord(SubWord(pre_col4)), int.to_bytes(Rcon[i+1], 4, 'little')))

        round_key = pre_col1 + pre_col2 + pre_col3 + pre_col4
    
    return round_key